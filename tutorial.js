(function () {
  ...
  // Ces données sont injectées par PHP via data- sur le body (ou fallback vide)
  const userId  = document.body.getAttribute("data-user-id")  || "";
  const token   = document.body.getAttribute("data-token")    || "";
  const autoStart = document.body.getAttribute("data-tutorial-autostart") === "1";
  ...
  // Marque le tutorial comme vu en BDD (appel API Go) + localStorage
  function markTutorialSeen() {
    if (window.localStorage) {
      localStorage.setItem("sh_tutorial_done", "1");
    }
    if (userId && token) {
      fetch("http://localhost:8080/api/users/" + encodeURIComponent(userId) + "/tutorial-seen", {
        method: "PUT",
  ...
  document.addEventListener("DOMContentLoaded", function () {
    // Déclenchement auto si PHP a injecté data-tutorial-autostart="1"
    // ou si l'ancienne clé localStorage est absente (utilisateur non connecté)
    if (autoStart) {
      openTutorial(true);
    } else if (window.localStorage && localStorage.getItem("sh_tutorial_autostart") === "1") {
      openTutorial(false);
    }
  });
})();
